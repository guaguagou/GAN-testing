...
num_pt = 58
pt_start = 391
data = ['./dataset/convWGAN-GP/path_10packet/Hyperg/F19/Hy1F1_gsl_sf_hyperg_1F1_F19.csv']
GAN_op_file = ['./dataset/convWGAN-GP/path_10packet/Hyperg/F19/']
...
while i < 10:
    ...
    while ii < pt_start+num_pt:
        reqd_Index = train_data[train_data['b{}'.format(ii)]>=2].index.tolist()
        if reqd_Index:
            ii=ii+1
        else:
            # The list is empty
    ...
    while iii < len(uncover_index):
        ...
        if a > 1:
            ...
            b_argmax.append(column[column == a].index.values)
        iii=iii+1
    if len(b_argmax)>0:
        #flatten
    ...
    a = os.system("python path_extract_10packet.py %d " % N)
...
#remove invalid data (can not execute the func under test)
    ...
    invalid_index = oneth_branch[oneth_branch == 0.5].index.values
    ...
    if len(valid_input_path)>0:
        ...
        new_train = old_train.append(valid_input_path)
